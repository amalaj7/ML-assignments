...
def persist_image(folder_path:str,url:str, counter):
    try:
        image_content = requests.get(url).content # go the url and get the binary format of the image

    except Exception as e:
        print(f"ERROR - Could not download {url} - {e}")

    try:
        f = open(os.path.join(folder_path, 'jpg' + "_" + str(counter) + ".jpg"), 'wb') # convert the image content to .jpg
...
def search_and_download(search_term: str, driver_path: str, target_path='./images', number_images=10):
    target_folder = os.path.join(target_path, '_'.join(search_term.lower().split(' '))) # this is the folder name , join try join strings
    #target_path + '/' + '_'.join(search_term.lower().split(' ')) - (if u dont want to use OS), this code may not work in windows,ubuntu,mac
    #if u are using os ,os tries to recognise the os which you are using and automatically form according to that and dont need to change the code.

    if not os.path.exists(target_folder):
        os.makedirs(target_folder) #create a folder/ directory

    with webdriver.Chrome(executable_path=driver_path) as wd: # opens a blank browser
        res = fetch_image_urls(search_term, number_images, wd=wd, sleep_between_interactions=0.5) # searches the term

    counter = 0
    for elem in res:
        persist_image(target_folder, elem, counter) # download image function
        counter += 1

# pip install -r requirements.txt

# My chrome Version 86.0.4240.183
# My Firefox Version 82.0.2 (64-bit)
...
# chrome version : Version 86.0.4240.183 (Official Build) (64-bit)
